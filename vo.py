...
class SimpleVO:
    ...
    def process_frames(self, queue):
        ...
        pre_frame_kp, pre_frame_des = orb.detectAndCompute(cv2.imread(self.frame_paths[0]), None)
        ...
        for idx,frame_path in enumerate(self.frame_paths[1:]):
            ...
            cur_frame_kp, cur_frame_des = orb.detectAndCompute(img, None)
            matches = bf.match(cur_frame_des, pre_frame_des) #pre_frame_des
            matches = sorted(matches, key = lambda x:x.distance)
            #step2
            pre_frame_points = np.array([pre_frame_kp[m.trainIdx].pt for m in matches])
            cur_frame_points = np.array([cur_frame_kp[m.queryIdx].pt for m in matches])
            ...
            #step4
            retval, R, t, mask, triangulated = cv2.recoverPose(E, pre_frame_points, cur_frame_points, \
